...
8. **Tucker-Decomposed Pure EML-KAN Layer Limitations**: Even when correcting implementation errors by introducing a linear base projection path and using FMA Matrix-Fused bottleneck powers ($Z, Z^2, Z^3$) to leverage GPU Tensor Cores, the SVD-factorized spatial projections ($U$ and $V$) still cause representation collapse (generating repetitive words) due to SVD information loss on untrained weights. Furthermore, executing multiple sequential GEMMs in the bottleneck space reduces throughput to **49.49 t/s** (slower than the dense compiled baseline of **57.21 t/s**) due to GPU kernel launch latency. This confirms that the full-rank dense compiled polynomial/rational implementation remains the optimal choice for reasoning correctness and throughput speed on GPU.
