...
| Model Variant / Optimization Configuration | Active Parameters | VRAM Footprint | GPU Throughput (t/s) | GPU Speedup (vs. Original BF16 = 59.46 t/s) | Coherence / Output Quality |
|:---|:---|:---|:---|:---|:---|
...
| **SDPA Attention + Ultra-Sparse DP-Collapsed Fused EML KAN (6-Block)** | 1.00 Billion | 2.24 GB | **63.80 t/s** | **1.07x (+7.3% Speedup)** | Output Drift (Uncalibrated) |
| **SDPA Attention + Fused GELU 4-Block DP-Collapsed EML KAN** | 1.00 Billion | 2.24 GB | **66.39 t/s** | **1.12x (+11.6% Speedup)** | Output Drift (Uncalibrated) |
...
3. **Why Multi-Layer DP Collapse (63.80 t/s & 66.39 t/s) Drifts Without Pretraining**:
   * Collapsing 26 MLP layers into 4 or 6 composite blocks bypasses intermediate Attention heads and LayerNorms across 6–7 merged layers.
   * Without a full 2,000-step pretraining run on a multi-GB text dataset, multi-layer polynomial composition error causes output token repetition drift (`make stick stick` or `????`).
