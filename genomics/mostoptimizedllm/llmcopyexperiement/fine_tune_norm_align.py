...
def run_norm_alignment():
    model_id = "google/gemma-3-1b-it"
    weights_path = "gemma3_eml_kan/model_state.pt"
    save_tuned_path = "gemma3_eml_kan/model_state_pure_aligned.pt"
    ...
    train_dataset = PackedGuanacoDataset(raw_dataset, tokenizer, seq_len=256, max_blocks=800)
    ...
    print("Swapping MLP blocks to PURE Gemma3EMLKANMLP (No Hybrid/Alpha Gating)...")
    ...
    # Optimize with AdamW at a learning rate of 3e-5
    ...
    print(f"Starting Norm-Aligned calibration training for {max_steps} steps...")
    t_start = time.time()
    ...
    print(f"Saving pure aligned weights to {save_tuned_path}...")
    torch.save(model.state_dict(), save_tuned_path)
    
    # Final eval
    model.eval()
    print("\n--- FINAL EVALUATION OF PURE EML-KAN MODEL (GPU) ---")
    response = run_evaluation(model, tokenizer, test_prompt)
    print(f"Response:\n{response}")
