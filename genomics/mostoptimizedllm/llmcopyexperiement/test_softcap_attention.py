...
class Gemma3HopfieldKANAttentionSoftcap(nn.Module):
    def __init__(self, original_attn):
        super().__init__()
        self.original_attn = original_attn
        self.attn_softcapping = getattr(original_attn, "attn_softcapping", None)
    ...
    def forward(self, hidden_states, position_embeddings=None, attention_mask=None, past_key_values=None, **kwargs):
        ...
        # Apply attention softcapping if configured
        if self.attn_softcapping is not None:
            scores = scores / self.attn_softcapping
...
def main():
    ...
    print("\n" + "="*80)
    print(" TESTING FULLY SWAPPED KAN MODEL (SOFTCAP ATTENTION + GATED EML-KAN MLP)")
    print("="*80)
    ...
    for i in range(model.config.num_hidden_layers):
        ...
        model.model.layers[i].self_attn = Gemma3HopfieldKANAttentionSoftcap(orig_attn).to(device)
