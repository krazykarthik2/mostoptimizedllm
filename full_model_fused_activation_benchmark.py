...
from verify_fused_glu_activation import FusedGLUPolynomialCompiler, CompiledFusedGELUMLP
from verify_fused_eml_attention import FusedHopfieldEMLAttention
from transformers import AutoTokenizer, AutoModelForCausalLM

class QuantizableFusedActivationGemma3EMLKANMLP(nn.Module):
    def __init__(self, config, layer_idx, state_dict, prune_threshold=1.5e-4, taylor_threshold=0.08):
        ...
        # Fit combined GELU + EML polynomials
        fused_compiler = FusedGLUPolynomialCompiler(dummy_layer, domain_bound=3.0)
        fused_coeffs = fused_compiler.fit_fused_activation(w_dict)
    ...
    def forward(self, x):
        ...
        # Fused Single-Pass Polynomial Activation (No GELU, no tanh!)
...
def main():
    ...
    print("Loading Gemma-3 with Fused GLU Activation + Hopfield Attention...")
    model = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch.float32, local_files_only=True)
    
    model.config._attn_implementation = "eager"
    
    for i in range(model.config.num_hidden_layers):
        # A. Replace MLP with compiled Fused Activation MLP
        compiled_mlp = QuantizableFusedActivationGemma3EMLKANMLP(
    ...
    # Compile graph with Inductor to apply Fused Activation logic
    ...
    new_row = (
        ...
        f"| **Fused GELU GLU + Hopfield Attention Model** | **{tps:.2f} t/s** | **{tps/1.98:.2f}x** | **Yes! {((tps/6.56) - 1.0)*100:.1f}% speedup over Quantized Original baseline!** |"
    )
